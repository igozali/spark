  ...
  private def loadEnvironmentArguments(): Unit = {
    ...
    if (mainClass == null && !isPython && !isR && primaryResource != null) {
      try {
        ...
        if (mainClass == null) {
          throw new RuntimeException(s"Cannot find Main-Class in manifest for JAR $primaryResource")
        }
      } catch {
        case exc: Exception =>
          throw new SparkException(
            s"Cannot load main class from JAR $primaryResource due to: ${exc.getMessage}")
      }
    }
